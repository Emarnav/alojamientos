...
const ApplicationCard = ({
  ...
  const [imgSrc, setImgSrc] = useState(
    solicitud.alojamiento.photoUrls?.[0] || "/placeholder.jpg"
  );
    ...
    <div className="border rounded-xl overflow-hidden shadow-sm bg-white mb-4">
      <div className="flex flex-col lg:flex-row  items-start lg:items-center justify-between px-6 md:px-4 py-6 gap-6 lg:gap-4">
        ...
        <div className="flex flex-col lg:flex-row gap-5 w-full lg:w-auto">
          <Image
            src={imgSrc}
            alt={solicitud.alojamiento.name}
            width={200}
            height={150}
          ...
          <div className="flex flex-col justify-between">
            <div>
              <h2 className="text-xl font-bold my-2">
                {solicitud.alojamiento.name}
              </h2>
              <div className="flex items-center mb-2">
                <MapPin className="w-5 h-5 mr-1" />
                <span>{`${solicitud.alojamiento.location.city}, ${solicitud.alojamiento.location.country}`}</span>
              </div>
            </div>
            <div className="text-xl font-semibold">
              ${solicitud.alojamiento.pricePerMonth}{" "}
        ...
        <div className="flex flex-col justify-between w-full lg:basis-2/12 lg:h-48 py-2 gap-3 lg:gap-0">
          ...
          <div className="flex justify-between">
            ...
            {new Date(solicitud.lease?.startDate).toLocaleDateString()}
          </div>
          <div className="flex justify-between">
          ...
          <div className="flex justify-between">
            ...
            {new Date(solicitud.lease?.nextPaymentDate).toLocaleDateString()}
          </div>
        </div>
        ...
        <div className="flex flex-col justify-start gap-5 w-full lg:basis-3/12 lg:h-48 py-2">
          ...
          <div className="flex gap-4">
            ...
            <div className="flex flex-col gap-2">
              <div className="font-semibold">{contactPerson.nombre}</div>
