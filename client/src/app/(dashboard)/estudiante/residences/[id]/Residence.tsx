...
import { Lease, Payment, Alojamiento } from "@/types/prismaTypes";
import {
  ArrowDownToLineIcon,
  Check,
  CreditCard,
      ...
      <div className="flex gap-5">
        ...
        <div className="flex flex-col justify-between">
          <div>
            ...
            <h2 className="text-2xl font-bold my-2">{alojamiento.name}</h2>
            <div className="flex items-center mb-2">
              <MapPin className="w-5 h-5 mr-1" />
              <span>
                {alojamiento.location.city}, {alojamiento.location.country}
              </span>
            </div>
          </div>
          <div className="text-xl font-bold">
            ${currentLease.rent}{" "}
            <span className="text-gray-500 text-sm font-normal">/ noche</span>
          </div>
        </div>
      </div>
      {/* Dates */}
...
const Residence = () => {
  ...
  if (!alojamiento || propertyError) return <div>Error loading property</div>;

  const currentLease = leases?.find(
    (lease) => lease.propertyId === alojamiento.id
  );
    ...
    <div className="dashboard-container">
      <div className="w-full mx-auto">
        <div className="md:flex gap-10">
          {currentLease && (
            <ResidenceCard alojamiento={alojamiento} currentLease={currentLease} />
          )}
          <PaymentMethod />
        </div>
