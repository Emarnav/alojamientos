...
const Favorites = () => {
  ...
  const {
    ...
    { favoriteIds: inquilino?.favoritos?.map((fav: { id: number }) => fav.id) },
    { skip: !inquilino?.favoritos || inquilino?.favoritos.length === 0 }
  );
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
        {favoriteProperties?.map((alojamiento) => (
          <Card
            ...
            alojamientoLink={`/inquilinos/alojamientos/${alojamiento.id}`}
          />
        ))}
      </div>
